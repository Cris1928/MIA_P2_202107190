...
def recorre(file, inicio, tipo, superblock):
    ...
    if inicio == -1:
       ...
       # print("objeto encontrado en "+str(inicio))
        publi.ress = publi.ress +"objeto encontrado en "+str(inicio)
        proceed, newbyte = espacio_libre_bitmap_bloques(file,superblock)
    file.seek(newbyte)
...
def get_file_content(partial_path):
    ...
    full_path = full_path= f'{partial_path}'
        ...
        for insidepath in insidepaths:
            ...
            if newI == -1:
               ...
               # print(f'byte del inodo {PI}')
                publi.ress = publi.ress +f'byte del inodo {PI}'
                file.seek(PI)
                inodo = Inode.unpack(file.read(Inode.SIZE))
                #print(inodo)
...
def remove(params, mounted_partitions,id, usuario_actual): 
    ...
    try: 
        ...
        #print("\nError:Se debe especificar la ruta.")
        publi.ress = publi.ress +"\nError:Se debe especificar la ruta."
        return
    partition = None
    for partition_dict in mounted_partitions:
   ...
   #current_directory = os.getcwd() #aqui se obtiene el directorio actual
    ...
    with open(full_path, "rb+") as file:
        ...
        #print(bitmap_bloques)
        #print("")
        #print(f'\narchivo {insidepath} eliminado')
        publi.ress = publi.ress +f'\narchivo {insidepath} eliminado'
        
    
        
def rename(params, mounted_partitions,id, usuario_actual):  
   # print(f'🏳️ \n>>EJECUTANDO RENAME {params}   ')
    publi.ress = publi.ress +f'🏳️ \n>>EJECUTANDO RENAME {params}   '
    if id == None:
        #print("\nError: Se necesita el id.")
        publi.ress = publi.ress +"\nError: Se necesita el id."
        return
    ...
    with open(full_path, "rb+") as file:
        ...
        for i,n in enumerate(lista_direcciones_destino):
            ...
            if esta:
                ...
                publi.ress = publi.ress +f'archivo {destinypath} no existe'
                return
        #print(PI_destino)
