...
class Journal:
    ...
    def __str__(self) -> str:
        publi.ress = publi.ress + f"\nJournal: data={self.journal_data.decode('utf-8')}"
...
class FolderBlock:
    ...
    def __init__(self):
        self.b_content = [Content("name_t", -1) for _ in range(4)]
    def __str__(self) -> str:
...
class FileBlock:
    ...
    def __init__(self):
        self.b_content = "name_t"
    def __str__(self) -> str:
        publi.ress = publi.ress + f"\nFileBlock: content={self.b_content}"
...
class Superblock:
    ...
    def __str__(self) -> str:
        publi.ress = publi.ress + f"\nSuperblock: filesystem_type={self.s_filesystem_type}, inodes_count={self.s_inodes_count}, blocks_count={self.s_blocks_count}, free_blocks_count={self.s_free_blocks_count}, free_inodes_count={self.s_free_inodes_count}, mtime={self.s_mtime}, umtime={self.s_umtime}, mnt_count={self.s_mnt_count}, magic={self.s_magic}, inode_s={self.s_inode_s}, block_s={self.s_block_s}, firts_ino={self.s_firts_ino}, first_blo={self.s_first_blo}, bm_inode_start={self.s_bm_inode_start}, bm_block_start={self.s_bm_block_start}, inode_start={self.s_inode_start}, block_start={self.s_block_start}"
    ...
    def ver_bytes_inidices(self):
     #   print(f"bm_inode_start={self.s_bm_inode_start}, \nbm_block_start={self.s_bm_block_start},  \ninode_start={self.s_inode_start},  \nblock_start={self.s_block_start} \n count_inodes={self.s_inodes_count} \n count_blocks={self.s_blocks_count}")
        publi.ress = publi.ress + f"\nbm_inode_start={self.s_bm_inode_start}, \nbm_block_start={self.s_bm_block_start},  \ninode_start={self.s_inode_start},  \nblock_start={self.s_block_start} \n count_inodes={self.s_inodes_count} \n count_blocks={self.s_blocks_count}"
        #input("especificaciones de ext y superbloque, presione enter para continuar ")
    @classmethod
    def unpack(cls, data):
        unpacked_data = struct.unpack(cls.FORMAT, data)
